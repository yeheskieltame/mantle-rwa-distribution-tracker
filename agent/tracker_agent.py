...
def token_snapshot(cfg, address, issuer):
    """Live snapshot from Routescan: exact holder count (paginated), top-1 %, issuer %."""
    base = "https://api.routescan.io"
    url = (f"{base}/v2/network/mainnet/evm/{cfg['chain_id']}"
           f"/erc20/{address}/holders?limit=100")
    items, pages = [], 0
    while url and pages < 10:  # ponytail: 10-page cap = exact up to 1000 holders, then lower bound
        d = get(url)
        if not d or not d.get("items"):
            break
        items += d["items"]
        pages += 1
        nxt = (d.get("link") or {}).get("next")
    ...
    return {
        "holders": len(items),
        "holders_exact": url is None,  # False = hit page cap, count is a lower bound
